    ...
    try:
        ...
        with database.get_session() as session:
            ...
            if fetch_results and fetch_results.get("papers_stored", 0) > 0:
                ...
                # Manual/CLI runs do not have Airflow XCom context, so index every paper
                # that already has parsed text instead of arbitrarily limiting to 24 hours.
                papers = session.query(Paper).filter(Paper.raw_text.isnot(None)).order_by(desc(Paper.created_at)).all()

            if not papers:
                logger.info("No papers with parsed text available to index for hybrid search")
