...
class DoclingParser:
    ...
    def _validate_pdf(self, pdf_path: Path) -> int:
        """Comprehensive PDF validation including size checks and page count discovery.

        :param pdf_path: Path to PDF file
        :returns: Actual page count for the PDF
        """
        try:
            ...
            # Discover page count so parsing can cap to the first max_pages pages if needed.
    ...
    def parse_pdf(self, pdf_path: Path) -> Optional[PdfContent]:
        ...
        Processes at most ``max_pages`` pages to avoid expensive parsing of references/appendices.

        :param pdf_path: Path to PDF file
        :returns: PdfContent object or None if parsing failed
        """
        try:
            # Validate PDF first and discover whether this parse will be truncated.
            actual_pages = self._validate_pdf(pdf_path)
            processed_pages = min(actual_pages, self.max_pages)
            was_truncated = actual_pages > self.max_pages

            if was_truncated:
                logger.info(
                    f"PDF has {actual_pages} pages; parsing first {self.max_pages} pages only and ignoring the remainder."
                )
            ...
            # Handle file-size validation errors gracefully by returning None
