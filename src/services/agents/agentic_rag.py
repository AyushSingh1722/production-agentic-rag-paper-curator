...
from .nodes.utils import count_retrieved_documents_from_tool_messages, extract_sources_from_tool_messages
...
class AgenticRAGService:
    ...
    async def _run_workflow(self, query: str, model_to_use: str, user_id: str, trace) -> dict:
        ...
        try:
            ...
            chunks_used = self._extract_chunks_used(result)
            ...
            trace_id = None
            if trace:
                trace_id = getattr(trace, "id", None) or self.langfuse_tracer.get_trace_id(trace)
    ...
    def _extract_sources(self, result: dict) -> List[str]:
        """Extract source URLs from graph result."""
        sources: List[str] = []
        seen_urls = set()
        ...
        if not sources:
            for source in extract_sources_from_tool_messages(result.get("messages", [])):
    ...
    def _extract_chunks_used(self, result: dict) -> int:
        """Extract actual retrieved chunk/document count from graph result."""
        messages = result.get("messages", [])
        return count_retrieved_documents_from_tool_messages(messages)
