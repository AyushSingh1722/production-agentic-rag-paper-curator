...
class RAGTracer:
    ...
    def trace_search(self, trace, query: str, top_k: int):
        ...
        with self.tracer.start_span(name="search_retrieval", input_data={"query": query, "top_k": top_k}) as span:
            yield span
    ...
    def trace_prompt_construction(self, trace, chunks: List[Dict]):
        ...
        with self.tracer.start_span(name="prompt_construction", input_data={"chunk_count": len(chunks)}) as span:
            yield span
    ...
    def trace_generation(self, trace, model: str, prompt: str):
        ...
        with self.tracer.start_span(
            name="llm_generation", input_data={"model": model, "prompt_length": len(prompt), "prompt": prompt}
        ) as span:
            yield span
