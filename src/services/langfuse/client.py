...
class LangfuseTracer:
    ...
    def create_span(
        ...
        """
        Backward-compatible span creation helper.

        The `trace` argument is accepted for compatibility with older agentic
        nodes, but the current v3 SDK usage does not require it explicitly here.
        """
        del trace
    ...
    def end_span(
        ...
        """Backward-compatible span finalizer."""
        self.update_span(
            span=span,
            output=output,
            metadata=metadata,
