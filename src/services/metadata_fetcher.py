...
from src.exceptions import MetadataFetchingException, PDFParsingException, PipelineException
        ...
        try:
            ...
            async with parse_semaphore:
                ...
                try:
                    ...
                    # Cached PDFs can become stale/corrupted; force one fresh download before failing.
                    if "corrupted" in str(e).lower() or "invalid" in str(e).lower():
                        logger.warning(f"Retrying {paper.arxiv_id} with a fresh PDF download after parse failure: {e}")
                        async with download_semaphore:
                            pdf_path = await self.arxiv_client.download_pdf(paper, True)
                            if not pdf_path:
                                raise
                        pdf_content = await self.pdf_parser.parse_pdf(pdf_path)
                    else:
    ...
    def _store_papers_to_db(
        ...
        for paper in papers:
            try:
                ...
                elif existing_paper and existing_paper.pdf_processed and existing_paper.raw_text:
                    # Preserve previously extracted content if a later reprocessing attempt fails.
                    paper_data.update(
                        {
                            "raw_text": existing_paper.raw_text,
                            "sections": existing_paper.sections,
                            "references": existing_paper.references,
                            "parser_used": existing_paper.parser_used,
                            "parser_metadata": existing_paper.parser_metadata,
                            "pdf_processed": existing_paper.pdf_processed,
                            "pdf_processing_date": existing_paper.pdf_processing_date,
                        }
                    )
                    logger.debug(f"Preserving existing parsed content for paper {paper.arxiv_id}")
                else:
