...
async def lifespan(app: FastAPI):
    ...
    if not settings.jina_api_key:
        logger.warning(
            "JINA_API_KEY is not set — embeddings will fail at query time. "
            "Hybrid search will not work. Set JINA_API_KEY in your .env file."
        )
    ...
    # Initialize Telegram bot only when explicitly enabled for in-process startup.
    # In docker-compose deployment, the bot should run in its own single-process service
    # to avoid polling conflicts with uvicorn workers.
    if settings.telegram.enabled and settings.telegram.run_in_api:
        ...
        logger.info("Telegram bot in API disabled - use dedicated bot service for deployment")
